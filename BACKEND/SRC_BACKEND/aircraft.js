...
exports.Rotta_a = void 0;
const express_1 = require("./express");
const express_2 = __importDefault(require("express"));
const UserDb_1 = require("./UserDb");
const Rotta_a = express_2.default.Router();
exports.Rotta_a = Rotta_a;
//AIRCRAFT
//POST /AIRCRAFT
/************************************************************************************************************/
...
Rotta_a.post("/aircraft", express_1.Sic_T, (req, res) => {
    let company = req.user;
    let aircraft = req.body.aircraft;
    if (company.role == "Airline") {
        UserDb_1.Users_DB.findOne({ email: company.email })
            .then((user) => {
            if (user === null || user === void 0 ? void 0 : user.Aircrafts.find((x) => x.id == aircraft.id))
                res.status(409).json({ message: "Aircraft già Presente" });
            else {
                UserDb_1.Users_DB.findOneAndUpdate({ email: company.email }, { $push: { Aircrafts: aircraft } }, { new: true })
                    .then((user) => {
                    if (user != null)
                        res.status(201).json({ message: "Aircraft Creato", aircraft: aircraft });
                    else
                        res.status(404).json({ message: "Errore nel salvataggio dell'Aircraft" });
                })
                    .catch((err) => {
...
Rotta_a.delete("/aircraft/:id", express_1.Sic_T, (req, res) => {
    let company = req.user;
    let flights = [];
    if (company.role == "Airline") {
        UserDb_1.Users_DB.findOneAndUpdate({ email: company.email }, { $pull: { Aircrafts: { id: parseInt(req.params.id) } } })
            .then((user) => {
            if (user != null) {
...
Rotta_a.get("/aircrafts", express_1.Sic_T, (req, res) => {
    let company = req.user;
    if (company.role == "Airline") {
        UserDb_1.Users_DB.findOne({ email: company.email })
            .then((user) => {
            if (user != null)
                res.send(user === null || user === void 0 ? void 0 : user.Aircrafts);
            else
                res.status(404).json({ message: "Utente non trovato" });
        })
            .catch(() => {
...
Rotta_a.put("/aircraft/:id", express_1.Sic_T, (req, res) => {
    let company = req.user;
    let aircraft = req.body.aircraft;
    if (company.role === "Airline") {
        UserDb_1.Users_DB.findOne({ email: company.email })
