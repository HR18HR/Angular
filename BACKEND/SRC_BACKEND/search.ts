...
import { Route_I, Flight_I, Aircraft_I, Users_DB,Ticket_I} from "./UserDb";
import crypto from 'crypto';
const Rotta_s = express.Router();
type Flight_s =Omit<Flight_I,'revenue Passengers state aircraft'>;
    ...
    .then((users) => {
      if (users.length === 0) {
        ...
        for (let user_1 of users) {
          let voliTrovati = user_1.Flights.filter((x: Flight_I) =>
            x.route.from == Cerca_F.from &&
...
Rotta_s.post("/purchase/flight/:id",Sic_T,(req,res)=>{
  let biglietto_d=<{n:number,type:string,extra_b:boolean,cost:number,DepartureTime:'',Date:'',route:{from:'',to:''}}>req.body.biglietto_d;
  let user_p=<User>req.user;
  let obje:Ticket_I={
    passanger:user_p.email,id_f:req.params.id,
    id_transaction:crypto.randomBytes(16).toString('hex'),
  ...
  if(user_p.role=="User"){
    Users_DB.findOne({"Flights.id":req.params.id})
    .then((user)=>{
      if(user==null)res.status(404).json({message:"Utente Non trovato"})
        else {
                if(user.Purchasing(req.params.id,{n:biglietto_d.n,type:biglietto_d.type},user_p.username)==false)res.status(400).json({message:"IL Numero di Biglietti selezionati va oltre la Disponibilià"})
                  else {
                    user.save()
                    .then((user)=>{
                      if(user==null)res.status(404).json({message:"Errore Interno nel salvataggio "})
                        let flight=<Flight_s>user.Flights.find((x)=>x.id==req.params.id);
                        Users_DB.findOneAndUpdate({email:user_p.email},{$push:{Tickets:obje}})
                        .then((user)=>{
                          if(user==null)res.status(404).json({message:"Errore nella creazione del Biglietto"});
                          else res.status(201).json({message:"Biglietto Acquistato",biglietto:obje,flight_p:flight})
                        })
...
else res.status(401).json({message:"Acesso Negato"})
})
